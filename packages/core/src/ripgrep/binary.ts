...
export namespace RipgrepBinary {
  ...
  const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // A location may close while the shared binary is being downloaded. Cache
      // the service-scoped fiber, not the first caller's interruptible wait.
      const resolving = yield* Effect.cached(
        Effect.gen(function* () {
      ...
      return Service.of({ filepath: resolving.pipe(Effect.flatMap(Fiber.join)) })
    }),
  )
