...
type Attempt = { key: string; createdAt: number; result?: Extract<ResetCardResult, { ok: true }> }
...
export class ResetCardClient {
  ...
  private async send(file: string, gateway: string, apiKey: string, id: number): Promise<ResetCardResult> {
    try {
      const attempt = this.read(file) ?? { key: randomUUID(), createdAt: Date.now() }
      if (attempt.result) return attempt.result
      // Never blindly replay beyond the server's idempotency retention period.
      if (Date.now() - attempt.createdAt > 23 * 60 * 60 * 1000) return { ok: false, error: "reconciliation_required" }
